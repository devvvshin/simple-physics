...
class BodySimulationVerletIntergration extends Component {
      ...
      points.forEach(({ currPos, prevPos, radius, lineWidth }, i) => {
        ...
        if (((radius + lineWidth) * 2 > dx) && ((radius + lineWidth) * 2 > dy)) {
          ...
          window.onmousemove = (e) => {
            if (selected) {
              let d = this.distance({ x: oldClientX, y: oldClientY}, { x: e.clientX, y: e.clientY });

              const dx =  (e.clientX - oldClientX) * 1.3;
              const dy =  (e.clientY - oldClientY) * 1.3;
  ...
  genBlobMeta() {
    ...
    const constraints = points.reduce((res, p, i, points) => {
      ...
      if (i < steps) { //for outer point
        ...
        const len = this.distance(points[baseIdx].currPos, points[centerIdx].currPos);
        res.push({
          len,
        ...
        if (baseIdx == points.length - 2) {  //last
          const len = this.distance(points[baseIdx].currPos, points[steps].currPos);
          res.push({
            len,
            link: [baseIdx, steps],
            elasticity: 0.5,
          ...
          const len = this.distance(points[baseIdx].currPos, points[baseIdx + 1].currPos);
          res.push({
            len,
            link: [baseIdx, baseIdx + 1],
            elasticity: 0.5,
  ...
  update(dt) {
    ...
    //constraints
    for (let i = 0; i < 100; i++) {
      constraints.forEach(({ link, len, elasticity, min, max }, i) => {
